;;; Sierra Script 1.0 - (do not remove this comment)
(script# 421)
(include sci.sh)
(use Main)
(use n021)
(use Class_255_0)
(use Cycle)
(use Obj)

(public
	ManagerScript 0
)

(local
	local0
	[local1 40]
	[local41 22]
)
(instance ManagerScript of Script
	(properties)
	
	(method (changeState newState)
		(proc21_1 self newState 4 2)
		(switch (= state newState)
			(0
				(Load rsVIEW 423)
				(Load rsSOUND 6)
				(Load rsSOUND 3)
				(Load rsSOUND 12)
			)
			(1
				(proc255_0 421 0)
				(gADoor setCycle: End self)
				(gTheMusic number: 6 loop: -1 play:)
			)
			(2
				(gADoor stopUpd:)
				(client setPri: 9 setCycle: Fwd posn: 33 143)
				(= seconds 3)
			)
			(3
				(proc255_0 421 1 67 10 5 70 290)
				(proc255_0 421 2 67 -1 144)
				(client loop: 1 cel: 0 setCycle: End self)
			)
			(4
				(gEgo hide:)
				(client loop: 2 cel: 0 setCycle: End self)
			)
			(5
				(proc255_0 421 3 67 -1 10 88)
				(gGame setSpeed: 1)
				(= seconds 3)
			)
			(6
				(proc0_5)
				(client
					loop: 3
					cycleSpeed: (Random 0 2)
					cel: 0
					setCycle: End self
				)
				(if (>= 10 (++ local0)) (-- state))
			)
			(7
				(client loop: 4 cel: 0 setCycle: End self)
			)
			(8
				(gEgo
					view: 421
					setLoop: 5
					posn: 60 128
					setStep: 20 9
					setPri: 11
					setMotion: MoveTo 312 68 self
					show:
				)
				(gTheMusic stop: number: 3 loop: 1 play:)
				(client loop: 0 cel: 0 stopUpd:)
				(global2 north: 0 east: 0)
			)
			(9
				(gTheMusic stop: number: 12 loop: 1 play:)
				(client hide:)
				(gADoor setCycle: Beg self)
			)
			(10
				(gGame setScript: (ScriptID 40))
				((ScriptID 40)
					caller: 423
					register: (Format @local1 421 4)
					next: (Format @local41 421 5)
				)
				(self dispose:)
			)
		)
	)
)
